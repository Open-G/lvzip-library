...
extern int ZEXPORT unzClose2(unzFile file, voidpf *output)
{
    unz64_internal *s;
    if (file == NULL)
...
extern int ZEXPORT unzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
    ...
    if ((password != NULL) && ((s->cur_file_info.flag & 1) != 0))
    {
...
#endif
        {
			...
			expected = (uint8_t)((s->cur_file_info.flag & (1 << 3)) ?
...
extern int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, uint32_t len)
{
    ...
    do
    {
        ...
        else if (s->pfile_in_zip_read->compression_method == Z_BZIP2ED)
        {
            ...
            s->pfile_in_zip_read->bstream.total_in_hi32  = (uint32_t)(s->pfile_in_zip_read->stream.total_in >> 32);
            ...
            s->pfile_in_zip_read->bstream.total_out_hi32 = (uint32_t)(s->pfile_in_zip_read->stream.total_out >> 32);

            total_out_before = s->pfile_in_zip_read->bstream.total_out_lo32 + 
                (((uint64_t)s->pfile_in_zip_read->bstream.total_out_hi32) << 32);
            ...
            s->pfile_in_zip_read->stream.total_in  = s->pfile_in_zip_read->bstream.total_in_lo32 + 
				                                     (((uint64_t)s->pfile_in_zip_read->bstream.total_in_hi32) << 32);
